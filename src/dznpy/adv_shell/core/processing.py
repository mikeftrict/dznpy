...
def create_cpp_port_helper_methods(label: str, cpp_ports: CppPorts, support_files_ns: NamespaceIds,
    ...
    """"create_cpp_helper_methods TODO"""
    public_fns = []
    private_fns = []
...
def initialize_port_impl(port: CppPortItf,
    ...
    tb1 = [chunk(initialize_port_impl1(port, support_files_ns))]
    tb2 = []
    tb3 = ['return port;']
    ...
    for event in [e for e in dzn.interface.events.elements if
        ...
        if event == dzn.multiclient.claim_event:
            tb2.append(initialize_port_impl2(port, dzn.multiclient, fc))
        elif event == dzn.multiclient.release_event:
            tb2.append(initialize_port_impl3(port, dzn.multiclient, fc))
        else:
...
def initialize_port_impl1(port: CppPortItf, support_files_ns: NamespaceIds) -> str:
...
def initialize_port_impl2(port: CppPortItf, multiclient: MultiClientPortCfgFixture,
...
def initialize_port_impl3(port: CppPortItf, multiclient: MultiClientPortCfgFixture,
                          fc: ast.FileContents) -> TextBlock:
