...
def create_dzn_primer(mocked_version: str) -> Tuple[DznPrimer, DznCommonOptions]:
    ...
    with patch('subprocess.run') as mock_run:
        ...
        return primer, DznCommonOptions(import_dirs=IMPORTS)


def test_create_dzn_primer_fail():
    """Test the bad weather scenario"""
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='', stderr='Error', returncode=1)
        with pytest.raises(RuntimeError) as exc:
            DznPrimer(dzn_path=Path(DZN_CMD_FILEPATH), root_folder=Path('models'))
        assert str(exc.value) == f'Failed to inquire {DZN_CMD_FILEPATH} its version. Error'
...
def test_create_dzn_parse_args_ok():
    ...
    assert create_dzn_parse_args(primer, common_opt, DznParseOptions(output_target=Path('-'), dzn_file=DZN_FILE)) == \
           [DZN_CMD_FILEPATH, 'parse', '-I', '../Inc', '-o', '-', 'model.dzn']

    assert create_dzn_parse_args(primer, common_opt, DznParseOptions(output_target=Path('gen/'), dzn_file=DZN_FILE)) == \
...
def test_create_dzn_parse_args_corner_cases():
    ...
    assert create_dzn_parse_args(primer, common_opt, DznParseOptions(list_models=True, preprocess=True,
                                                                     output_target=Path('gen/'), dzn_file=DZN_FILE)) == \
...
def test_create_dzn_code_args_ok():
    ...
    assert create_dzn_code_args(primer, common_opt, DznCodeOptions(output_target=Path('gen/'), dzn_file=DZN_FILE)) == \
    ...
    assert create_dzn_code_args(primer, common_opt, DznCodeOptions(tss=['My.Toaster', 'Something.Else'],
                                                                   output_target=Path('gen/'), dzn_file=DZN_FILE)) == \
           [DZN_CMD_FILEPATH, 'code', '-I', '../Inc', '--shell=My.Toaster', '--shell=Something.Else', '-o', 'gen', 'model.dzn']
...
def test_dzn_version_ok():
    ...
    with patch('subprocess.run') as mock_run:
        ...
        assert result.proc.cmdline == f'{DZN_CMD_FILEPATH} --version'


def test_dzn_version_fail():
    """Test a failing scenario where the process responds failure."""
    ...
    with patch('subprocess.run') as mock_run:
        ...
        assert result.proc.stderr == 'Error'
        assert result.message == 'Requesting Dezyne version failed'


def test_dzn_list_models_ok():
    """Test a good weather scenario."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout=DZN_PARSE_L_OUTPUT, stderr='', returncode=0)
        result = dzn_list_models(primer, IMPORTS, DZN_FILE)
        assert result.proc.succeeded() == True
        assert result.proc.stdout == DZN_PARSE_L_OUTPUT
        assert result.proc.cmdline == f'{DZN_CMD_FILEPATH} parse -I ../Inc --list-models {DZN_FILE}'


def test_dzn_list_models_fail():
    """Test a failing scenario where the process responds failure."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='', stderr='Error', returncode=1)
        result = dzn_list_models(primer, IMPORTS, DZN_FILE)
        assert result.proc.succeeded() == False
        assert result.proc.exit_code == 1
        assert result.proc.stderr == 'Error'
        assert result.message == 'Listing Dezyne file models failed'


def test_dzn_json_ok():
    """Test a good weather scenario."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout=DZNFILE_EXAMPLE, stderr='', returncode=0)
        result = dzn_json(primer, IMPORTS, DZN_FILE)
        assert result.proc.succeeded() == True
        assert result.proc.stdout == DZNFILE_EXAMPLE
        assert result.proc.cmdline == f'{DZN_CMD_FILEPATH} code -I ../Inc --language=json -o - {DZN_FILE}'


def test_dzn_json_fail():
    """Test a failing scenario where the process responds failure."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='', stderr='Error', returncode=1)
        ...
        assert result.proc.exit_code == 1
        assert result.proc.stderr == 'Error'
        assert result.message == 'Getting JSON AST for Dezyne file failed'


def test_dzn_parse_preprocess_ok():
    """Test a good weather scenario."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout=DZN_PARSE_PREPROCESS_OUTPUT, stderr='', returncode=0)
        common_opt = DznCommonOptions(import_dirs=IMPORTS)
        parse_opt = DznParseOptions(preprocess=True, dzn_file=DZN_FILE)
        result = dzn_parse(primer, common_opt, parse_opt)
        assert result.proc.succeeded() == True
        assert result.proc.stdout == DZN_PARSE_PREPROCESS_OUTPUT
        assert result.proc.cmdline == f'{DZN_CMD_FILEPATH} parse -I ../Inc --preprocess {DZN_FILE}'


def test_dzn_parse_preprocess_fail1():
    """Test a failing scenario where the process responds failure."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='', stderr='Error', returncode=1)
        ...
        assert result.proc.stderr == 'Error'
        assert result.message == 'Parsing Dezyne file failed'


def test_dzn_parse_preprocess_fail2():
    """Test a failing scenario where an option is requested while it is not supported."""
    primer, _ = create_dzn_primer(V2_11_0)  # intentionally choosing an 'old' version of dezyne
    common_opt = DznCommonOptions(skip_wfc=True)  # skip-wfc not supported in 2.11
    parse_opt = DznParseOptions(dzn_file=DZN_FILE)
    result = dzn_parse(primer, common_opt, parse_opt)
    assert result.proc.succeeded() == False
    assert result.proc.exit_code == 2
    assert result.proc.stdout == ''
    assert result.proc.stderr == ''
    assert result.proc.message is None
    assert result.message == 'Error in specified options; --skip-wfc is only supported as of dezyne 2.12.0 args: --skip-wfc'


def test_dzn_code_ok():
    """Test a good weather scenario."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='int i=0;', stderr='', returncode=0)
        common_opt = DznCommonOptions(import_dirs=IMPORTS)
        code_opt = DznCodeOptions(dzn_file=DZN_FILE, language='c++', output_target=Path('-'))
        result = dzn_code(primer, common_opt, code_opt)
        assert result.proc.succeeded() == True
        assert result.proc.stdout == 'int i=0;'
        assert result.proc.cmdline == f'{DZN_CMD_FILEPATH} code -I ../Inc --language=c++ -o - {DZN_FILE}'


def test_dzn_code_fail1():
    """Test a failing scenario where the process responds failure."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='', stderr='Error', returncode=1)
        common_opt = DznCommonOptions(import_dirs=IMPORTS)
        code_opt = DznCodeOptions(dzn_file=DZN_FILE, language='c++')
        result = dzn_code(primer, common_opt, code_opt)
        assert result.proc.succeeded() == False
        assert result.proc.exit_code == 1
        assert result.proc.stdout == ''
        assert result.proc.stderr == 'Error'
        assert result.message == 'Generate code for Dezyne file failed'


def test_dzn_code_fail2():
    """Test a failing scenario where an option is requested while it is not supported."""
    primer, _ = create_dzn_primer(V2_11_0)  # intentionally choosing an 'old' version of dezyne
    common_opt = DznCommonOptions(skip_wfc=True)  # skip-wfc not supported in 2.11
    code_opt = DznCodeOptions(dzn_file=DZN_FILE, language='c++')
...
def test_dzn_verify_ok():
    """Test a good weather scenario."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='', stderr='', returncode=0)
        common_opt = DznCommonOptions(import_dirs=IMPORTS)
        verify_opt = DznVerifyOptions(dzn_file=DZN_FILE)
        result = dzn_verify(primer, common_opt, verify_opt)
        assert result.proc.succeeded() == True
        assert result.proc.stdout == ''
        assert result.proc.stderr == ''
        assert result.proc.cmdline == f'{DZN_CMD_FILEPATH} verify -I ../Inc {DZN_FILE}'


def test_dzn_verify_fail1():
    """Test a failing scenario where the process responds failure."""
    primer, _ = create_dzn_primer(V2_19_0)
    with patch('subprocess.run') as mock_run:
        mock_run.return_value = MagicMock(stdout='', stderr='Error', returncode=1)
        ...
        assert result.proc.stdout == ''
        assert result.proc.stderr == 'Error'
        assert result.message == 'Verifying Dezyne file failed'


def test_dzn_verify_fail2():
    """Test a failing scenario where an option is requested while it is not supported."""
    primer, _ = create_dzn_primer(V2_11_0)  # intentionally choosing an 'old' version of dezyne
    common_opt = DznCommonOptions(verbose=True)  # skip-wfc not supported in 2.11
    verify_opt = DznVerifyOptions(no_constraint=True, dzn_file=DZN_FILE)
    result = dzn_verify(primer, common_opt, verify_opt)
    assert result.proc.succeeded() == False
    assert result.proc.exit_code == 2
    assert result.proc.stdout == ''
    assert result.proc.stderr == ''
    assert result.proc.message is None
    assert result.message == f'Error in specified options; --no-constraint is only supported as of dezyne 2.17.0 args: {DZN_CMD_FILEPATH} --verbose verify --no-constraint model.dzn'
